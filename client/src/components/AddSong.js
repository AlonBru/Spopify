...
function AddSong({ sendData, sendStatus, showData }) {
  ...
  const submitChecks = (e) => {
    ...
    if (!dataToAdd.artist) {
      ...
      e.target.reset();
      const { album_id } = dataToAdd.album;
  ...
  return (
    ...
    >
      ...
      <div>
        <label htmlFor="name">
          song title*:
          <input required name="name" placeholder="song title" onChange={inputChange} />
          {' '}
          <br />
        </label>
        <label htmlFor="youtube_link">
          youtube link*:
          <input required name="youtube_link" placeholder="youtube link" onChange={inputChange} />
          {' '}
          <br />
        </label>
        ...
        <label htmlFor="released">
          release date (for singles, album automatically adds this):
